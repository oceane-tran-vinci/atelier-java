...
public class Camion  {
    ...
    //TODO trouverTrajetsAvecPlaceRestante() et trouverDateTrajet
    /**
     ...
     * par ville d'arrivée
     */
    //On a : une liste dc doit => .collect() à la fin
    //nbMaxCaisses pas encore atteint => .filter(e < nbMaxCaisses)
    //"classé" => .sorted
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets
                .stream()
                .filter( e -> e.nbCaisses() < nbMaxCaisses)
                .sorted(Comparator.comparing(Trajet::getVilleDepart).thenComparing(Trajet::getVilleArrivee))
     ...
     */
    //"renvoie la date du trajet" => .map()
    //"la date du premier" => .reduce
    //"date trajet ultérieure à la date d'aujourd'hui"
    // + "ayant comme ville de départ celle passée en paramètre"
    // + "ayant comme ville d'arrivée celle passée en paramètre"  => .filter()
    //"Renvoie null s'il n'y a pas de trajet correspondant" .orElse (peut faire Optional mais peut pas avant return,
    //sinon aurait fait "public Optional<LocalDate> trouverDateTrajet..."
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets
                .stream()
                .filter(t -> t.getDate().isAfter(LocalDate.now()))//la date du premier trajet ultérieure à la date d'aujourd'hui
                .filter(t -> t.getVilleDepart().equals(villeDepart))//ayant comme ville de départ celle passée en paramètre
                .filter(t -> t.getVilleArrivee().equals(villeDArrivee))//ayant comme ville d'arrivée celle passée en paramètre
                .reduce((t1, t2) -> t1.getDate().isBefore(t2.getDate()) ? t1 : t2)// Trouve le trajet avec la date la plus proche
                //autre manière .min(Comparator.comparing(Trajet::getDate))
                .map(Trajet::getDate)
                .orElse(null);
    }
